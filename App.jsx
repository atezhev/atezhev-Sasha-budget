...
let RATES = {RUB:1,TRY:3.2,USD:90,EUR:98,GBP:114};
...
export default function App() {
  ...
  const [sources, setSources] = useState(() => LS.get("sources", DEF_SOURCES));
  const [accounts, setAccounts] = useState(() => LS.get("accounts", DEF_ACCOUNTS));
  const [cats, setCats] = useState(() => LS.get("cats", DEF_CATS));
  const [txs, setTxs] = useState(() => LS.get("txs", DEF_TXS));
  const [rates, setRates] = useState(() => LS.get("rates", RATES_DEFAULT));
  ...
  useEffect(() => { LS.set("sources", sources); }, [sources]);
  useEffect(() => { LS.set("accounts", accounts); }, [accounts]);
  useEffect(() => { LS.set("cats", cats); }, [cats]);
  useEffect(() => { LS.set("txs", txs); }, [txs]);
  ...
  // fetch real rates from CBR on load
  useEffect(() => {
    fetch("https://www.cbr-xml-daily.ru/daily_json.js")
      .then(r => r.json())
      .then(json => {
        const newRates = { RUB: 1 };
        ["TRY","USD","EUR","GBP"].forEach(code => {
          if (json.Valute?.[code]) newRates[code] = parseFloat((json.Valute[code].Value / json.Valute[code].Nominal).toFixed(4));
        });
        setRates(newRates);
  ...
  return (
    <div style={{ background: "#0a0a10", minHeight: "100vh", color: "#f0ede8", fontFamily: "'Nunito',sans-serif", maxWidth: 480, margin: "0 auto", paddingBottom: 80, overflowX: "hidden" }}>
      <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&display=swap" rel="stylesheet" />
      <div style={{ background: "linear-gradient(90deg,#7c6bff,#ff6b9d)", padding: "7px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 13, fontWeight: 800 }}>💰 Бюджет Саши</span>
        <span style={{ fontSize: 11, opacity: 0.85 }}>{rateDate ? `1₺ = ${rates.TRY}₽ · ${rateDate}` : "загрузка курса..."}</span>
      </div>
      {toast && (
      ...
      <nav style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#0e0e1a", borderTop: "1px solid #1e1e2a", display: "flex", zIndex: 100 }}>
...
// ── HOME ──────────────────────────────────────────────────────────────────────
function HomeTab({ sources, setSources, accounts, setAccounts, cats, setCats, txs, addTx, catSpentRub, catSpentCur, srcRcvRub, totalBudRub, totalSpentRub, totalExpected, totalReceived, totalWealthRub, pct, barCol, rates, rateDate, onPanel, showToast }) {
...
function Sheet({ title, onClose, actions, children }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "flex-end" }}
      ...
      <div style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#1a1a28", borderRadius: "22px 22px 0 0", maxHeight: "92vh", overflowY: "auto" }}>
