...
export default function App() {
  ...
  // Multi-month data: { "2025-06": { expenses: [], transfers: [] }, ... }
  const [allMonths, setAllMonths] = useState(LS.get("allMonths", {}));
  ...
  useEffect(() => {
    if (!allMonths[curKey]) {
      setAllMonths(prev => {
        const next = { ...prev, [curKey]: { expenses: [], transfers: [] } };
        LS.set("allMonths", next);
        return next;
      });
    }
  }, [curKey]);

  const monthData = allMonths[viewMonth] || { expenses: [], transfers: [] };
  const expenses = monthData.expenses;
  const transfers = monthData.transfers;

  const setExpenses = (updater) => {
    setAllMonths(prev => {
      const cur = prev[viewMonth] || { expenses: [], transfers: [] };
      const next = { ...prev, [viewMonth]: { ...cur, expenses: typeof updater === "function" ? updater(cur.expenses) : updater } };
      LS.set("allMonths", next);
      return next;
    });
  };

  const setTransfers = (updater) => {
    setAllMonths(prev => {
      const cur = prev[viewMonth] || { expenses: [], transfers: [] };
      const next = { ...prev, [viewMonth]: { ...cur, transfers: typeof updater === "function" ? updater(cur.transfers) : updater } };
      LS.set("allMonths", next);
      return next;
    });
  };
  ...
  const isCurrentMonth = viewMonth === curKey;
  const allMonthKeys = Object.keys(allMonths).sort().reverse();
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0f0f14", color: "#f0ede8", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito', sans-serif", padding: "20px" }}>
        ...
        <div style={{ width: "100%", maxWidth: 420, background: "#18181f", borderRadius: 24, padding: 32, boxShadow: "0 8px 40px rgba(0,0,0,0.4)" }}>
          ...
          <p style={{ textAlign: "center", color: "#888", marginBottom: 28, fontSize: 14 }}>Первый запуск</p>
    ...
    <div style={{ minHeight: "100vh", background: "#0f0f14", color: "#f0ede8", fontFamily: "'Nunito', sans-serif", maxWidth: 480, margin: "0 auto", paddingBottom: 80 }}>
      ...
      {allMonthKeys.length > 1 && (
        <div style={{ padding: "12px 16px 0", display: "flex", gap: 8, overflowX: "auto" }}>
          {allMonthKeys.map(k => (
            <button key={k} onClick={() => setViewMonth(k)}
              style={{ whiteSpace: "nowrap", padding: "6px 14px", borderRadius: 20, border: "none", background: viewMonth === k ? "#7c6bff" : "#252530", color: viewMonth === k ? "#fff" : "#aaa", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              {monthLabel(k)}{k === curKey ? " ●" : ""}
            </button>
          ))}
        </div>
      )}
      ...
      {!isCurrentMonth && (
        <div style={{ margin: "10px 16px 0", padding: "10px 14px", background: "#2a1f10", borderRadius: 12, fontSize: 13, color: "#fbbf24" }}>
          📅 Просмотр архива: {monthLabel(viewMonth)}. Расходы можно добавлять только в текущем месяце.
        </div>
      )}

      {tab === "home" && <HomeTab expenses={expenses} wallets={wallets} rate={rate} rateDate={rateDate} rateLoading={rateLoading} fetchRate={fetchRate} setRate={setRate} totalSpent={totalSpent} totalLeft={totalLeft} spentByCategory={spentByCategory} walletTotalRub={walletTotalRub} viewMonth={viewMonth} isCurrentMonth={isCurrentMonth} />}
      {tab === "add" && <AddExpenseTab expenses={expenses} setExpenses={setExpenses} wallets={wallets} setWallets={setWallets} rate={rate} showToast={showToast} toRub={toRub} onDone={() => setTab("home")} isCurrentMonth={isCurrentMonth} />}
      {tab === "list" && <ExpenseListTab expenses={expenses} setExpenses={setExpenses} wallets={wallets} setWallets={setWallets} toRub={toRub} showToast={showToast} isCurrentMonth={isCurrentMonth} />}
      {tab === "transfer" && <TransferTab transfers={transfers} setTransfers={setTransfers} wallets={wallets} setWallets={setWallets} rate={rate} showToast={showToast} onDone={() => setTab("home")} isCurrentMonth={isCurrentMonth} />}
      {tab === "report" && <ReportTab expenses={expenses} transfers={transfers} wallets={wallets} rate={rate} rateDate={rateDate} toRub={toRub} totalSpent={totalSpent} totalLeft={totalLeft} spentByCategory={spentByCategory} showToast={showToast} viewMonth={viewMonth} />}
...
// ─── HOME TAB ─────────────────────────────────────────────────────────────────
function HomeTab({ expenses, wallets, rate, rateDate, rateLoading, fetchRate, setRate, totalSpent, totalLeft, spentByCategory, walletTotalRub, viewMonth, isCurrentMonth }) {
  const [editRate, setEditRate] = useState(false);
  const [rateInput, setRateInput] = useState(rate);
  const pct = Math.min(100, (totalSpent / TOTAL_BUDGET) * 100);
  const barColor = pct > 100 ? "#ff4d6d" : pct > 70 ? "#fbbf24" : "#4ade80";

  return (
    <div style={{ padding: "16px 16px 16px" }}>
      <div style={{ marginBottom: 16 }}>
        <p style={{ color: "#888", fontSize: 13, marginBottom: 2 }}>Бюджет Саши · {monthLabel(viewMonth)}</p>
        {isCurrentMonth && (
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, margin: 0 }}>
            {daysLeftInMonth()} дней до конца месяца
          </h1>
        )}
      </div>
      ...
      <div style={{ background: "#18181f", borderRadius: 20, padding: "20px 18px", marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
          <div>
            ...
            <p style={{ fontWeight: 900, fontSize: 24, margin: 0 }}>{fmtRUB(totalSpent)}</p>
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
          ...
          <span style={{ color: "#666", fontSize: 11 }}>Бюджет: {fmtRUB(TOTAL_BUDGET)} · {Math.round(pct)}%</span>
        </div>
      </div>
...
// ─── ADD EXPENSE ──────────────────────────────────────────────────────────────
function AddExpenseTab({ expenses, setExpenses, wallets, setWallets, rate, showToast, toRub, onDone, isCurrentMonth }) {
  const [form, setForm] = useState({ date: today(), amount: "", currency: "TRY", category: "food", account: "turkish", comment: "" });
  ...
  if (!isCurrentMonth) {
    return (
      <div style={{ padding: "40px 16px", textAlign: "center", color: "#888" }}>
        ...
        <p>Расходы можно добавлять только в текущем месяце.</p>
        <p style={{ fontSize: 13 }}>Выбери текущий месяц в переключателе вверху.</p>
      </div>
    );
  }
  ...
  const submit = () => {
    ...
    setExpenses(prev => [{ id: Date.now(), ...form, amount: amt }, ...prev]);
    setWallets(w => {
      const next = { ...w };
...
// ─── EXPENSE LIST ──────────────────────────────────────────────────────────────
function ExpenseListTab({ expenses, setExpenses, wallets, setWallets, toRub, showToast, isCurrentMonth }) {
...
// ─── TRANSFER TAB ──────────────────────────────────────────────────────────────
function TransferTab({ transfers, setTransfers, wallets, setWallets, rate, showToast, onDone, isCurrentMonth }) {
  ...
  const calcRate = () => {
    const from = parseFloat(form.amtFrom), to = parseFloat(form.amtTo);
  ...
  const submit = () => {
    ...
    setTransfers(prev => [{ id: Date.now(), ...form, amtFrom: f, amtTo: t, exchRate: calcRate() }, ...prev]);
    if (isCurrentMonth) {
      setWallets(w => {
        const next = { ...w };
    ...
    <div style={{ padding: "24px 16px 16px" }}>
      ...
      {!isCurrentMonth ? (
        <div style={{ textAlign: "center", color: "#888", padding: "20px 0" }}>
          <p>Переводы можно добавлять только в текущем месяце.</p>
        </div>
      ) : (
        <>
          <Field label="Дата">
          ...
          {calcRate() && <p style={{ color: "#7c6bff", fontSize: 13, marginBottom: 12 }}>Курс обмена: {calcRate()} ₽/₺</p>}
...
// ─── REPORT TAB ───────────────────────────────────────────────────────────────
function ReportTab({ expenses, transfers, wallets, rate, rateDate, toRub, totalSpent, totalLeft, spentByCategory, showToast, viewMonth }) {
  ...
  const copyReport = () => {
    const lines = [`${monthLabel(viewMonth)} — отчёт по расходам Саши\n`];
  ...
  return (
    <div style={{ padding: "24px 16px 16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, margin: 0 }}>Отчёт · {monthLabel(viewMonth)}</h1>
      ...
      <div style={{ background: "#18181f", borderRadius: 18, overflow: "hidden", marginBottom: 16 }}>
        ...
        <div style={{ padding: "12px 14px", borderTop: "2px solid #7c6bff", display: "grid", gridTemplateColumns: "1fr 80px 80px 60px", gap: 4 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 12, color: totalSpent > TOTAL_BUDGET ? "#ff4d6d" : "#f0ede8" }}>{Math.round(totalSpent).toLocaleString("ru")} ₽</span>
      ...
      <div style={{ background: "#18181f", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
        ...
        {[["С турецкой карты", fmtRUB(trkRub)], ["С Тинькофф", fmtRUB(tinkRub)], ["В лирах", fmtTRY(inTry)], ["В рублях", fmtRUB(inRub)]].map(([k, v]) => (
      ...
      <div style={{ background: "#18181f", borderRadius: 16, padding: "16px" }}>
